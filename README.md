...
4. Set pre-deploy command:
   ```bash
   (./vendor/bin/drush status --field=bootstrap 2>/dev/null | grep -q Successful || ./vendor/bin/drush site:install -y) && ./vendor/bin/drush updatedb -y && ([ -f config/sync/core.extension.yml ] && ./vendor/bin/drush config:import -y || true) && ./vendor/bin/drush cache:rebuild
   ```
5. Deploy.
...
3. Update pre-deploy command:
   ```bash
   mkdir -p /app/web/sites/default && ln -sfn "$VOLUME_PATH" /app/web/sites/default/files && (./vendor/bin/drush status --field=bootstrap 2>/dev/null | grep -q Successful || ./vendor/bin/drush site:install -y) && ./vendor/bin/drush updatedb -y && ([ -f config/sync/core.extension.yml ] && ./vendor/bin/drush config:import -y || true) && ./vendor/bin/drush cache:rebuild
   ```
...
This creates YAML files in `config/sync/`. Once committed, the pre-deploy command will automatically import them.
...
- Config import only runs if `config/sync/core.extension.yml` exists.
- **Admin credentials** are shown in the deployment logs on first install (look for `User name: admin User password: ...`).
